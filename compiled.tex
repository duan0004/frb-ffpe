...
\label{eq:alpha}
\end{equation}
as confirmed across our full simulation grid (Methods). This scaling holds in the moderate-$\kappa/\sigma$ regime explored here; in the limit $\kappa \to 0$ (pure noise) $\alpha \to 1$ and no power-law tail emerges, while for $\kappa \gg \sigma^2$ the cubic potential term dominates and the linear approximation breaks down. Within the astrophysically relevant range $\kappa/\sigma^2 \in [0.1, 1.5]$, the relation is robust ($R^2 > 0.99$). Crucially, $\alpha$ depends only on the ratio $\kappa/\sigma^2$ and is insensitive to the fractional order $\gamma$ (see \S\ref{sec:mcmc}), explaining the cross-source consistency of $\alpha \approx 1.8$--$2.1$.
...
\label{eq:k_gamma}
\end{equation}
(Methods). This numerical relation, validated over the range $\gamma \in [0.3, 0.95]$, transforms the observed Weibull $k$ into an estimate of the fractional order $\gamma$. At the boundaries, the linear approximation slightly underestimates $k$ for $\gamma < 0.3$ (where very few FRB sources are expected) and yields $k \approx 0.77$ at $\gamma = 0.95$ rather than the Poisson limit $k = 1$, reflecting the residual effect of the first-passage coupling.
...
We note that $\gamma_{\mathrm{CHIME}} = 0.452$ for FRB~20220912A differs from $\gamma = 0.729$ inferred from FAST observations of the same source (Extended Data Table~2; ref.~\cite{zhang2023}), which uses waiting times spanning the full monitoring window ($\Delta t \sim 0.01$--$1{,}000$\,s). This discrepancy is attributable to timescale-dependent memory: magnetospheric trapping operates on a hierarchy of characteristic timescales, and the effective memory sampled depends on the observation window. Sub-minute intervals (CHIME transit mode) probe predominantly the deepest, fastest-relaxing traps, yielding a lower effective $\gamma$ (stronger non-Markovian clustering). Hours-long sessions (FAST) average over both fast and slow relaxation processes, producing a higher effective $\gamma$ closer to the Markov limit. This behaviour is analogous to multi-scale turbulence, where the effective Hurst exponent depends on the temporal resolution of the measurement. The timescale dependence may itself encode additional physical information about the magnetospheric trapping hierarchy.
...
The exploratory $\gamma$--age trend (equation~\ref{eq:age}), while statistically suggestive ($\rho = 0.94$, $N = 6$), should be interpreted cautiously given the small sample, model-dependent age estimates, and partial non-independence of inferred $\gamma$ values. To quantify robustness, we perform a Monte Carlo perturbation test: drawing 10{,}000 realizations in which each $\gamma$ is perturbed within its measurement uncertainty and each age is sampled log-uniformly within its estimated range, the median Spearman correlation remains $\rho_{\mathrm{MC}} = 0.83$ (95\% CI: $[0.43, 1.00]$), with $\rho > 0.7$ in 79\% and $p < 0.05$ in 58\% of realizations. Thus, while the correlation is not conclusive at $N = 6$, it is unlikely to be a statistical artifact of the age uncertainties alone. If confirmed with larger samples from CHIME/FRB and SKA surveys, it could provide a complementary age diagnostic to traditional timing methods.
...
Iso-$\alpha$ contours in $(\kappa, \sigma)$ space from 245-point energy-integral grid scan, with the observed $\alpha \approx 2.1$ contour (dashed) passing through the MCMC posterior region (panel a). Calibration relation $k = 0.85\gamma - 0.04$ with three MCMC-constrained sources marked; error bars denote 68\% credible intervals (panel b). Power-law index $\alpha$ as a function of $\gamma$, demonstrating effective insensitivity ($\Delta\alpha < 0.02$)---$\alpha$ is insensitive to $\gamma$ within the modeled regime (panel c). Residuals $|\mathrm{predicted} - \mathrm{observed}|$ for $\alpha$ and $k$ across three sources (panel d).}
\label{fig:mcmc}
\end{figure*}
...
Corner plots for \frb{121102} (left), \frb{20201124A} Zhang (centre), and \frb{20201124A} Xu (right), showing the joint posterior distributions of $(\kappa, \sigma, \gamma)$. Contours denote 68\% and 95\% credible regions. The fractional order $\gamma$ is the most directly constrained parameter via $k$ ($\sigma_\gamma \approx 0.06$), while $\kappa$ and $\sigma$ exhibit a partial degeneracy along the $\alpha \propto \kappa/\sigma^2$ direction.}
\label{fig:corner}
\end{figure*}
...
The FAST FRB~121102 data are publicly available from VizieR (catalogue J/Nature/598/267). The FRB~20201124A data are available from Figshare (doi:10.6084/m9.figshare.20432920). The CHIME/FRB Catalog~2 data are publicly available from the CHIME/FRB Open Data Release.

% ============================================================
% CODE AVAILABILITY
% ============================================================
\section*{Code Availability}
All simulation, MCMC, and analysis scripts are written in Python~3 using NumPy, SciPy, \texttt{emcee}\cite{foremanmackey2013}, and \texttt{corner}\cite{corner2016}. Source code is archived at \url{https://github.com/duan0004/frb-ffpe} and will be made public upon acceptance.
...
\bibitem{chimefrbcat2} CHIME/FRB Collaboration. CHIME/FRB Catalog 2: The Second CHIME/FRB Fast Radio Burst Catalog. Preprint at \url{https://arxiv.org/abs/2501.13535} (2025).
